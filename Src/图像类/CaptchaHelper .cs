...
namespace Kane.Extension
{
    ...
    public class CaptchaHelper
    {
        ...
        {
            if (path.IsValuable() && File.Exists(path))
            {
        ...
        public (Bitmap Image, string Answer) GetCaptcha(string code = null)
        {
            ...
            if (string.IsNullOrEmpty(code))//当code为空时，即按照【VerificationCodeMode】生成相应该字符串
            {
                ...
                {
                    code = result = RandomHelper.RandomChinese(ChineseLength);
                }
            ...
            for (int i = 0; i < bitmap.Width * NoisePointCount; i++)
            {
                bitmap.SetPixel(random.Next(bitmap.Width), random.Next(bitmap.Height),
                    RandomBackgroundColor ? UseDarkMode ? RandomHelper.RandomLightColor() : RandomHelper.RandomDarkColor() : FontColor);
            }
            //是否扭曲图片
            if (ToTwistImage) return (ImageHelper.TwistImage(bitmap, TwistSetting.XDirection, TwistSetting.MultValue, TwistSetting.Phase, backgroundColor), result);
